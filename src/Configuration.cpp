...
void Configuration::check_commandline(){
  ...
  if(cl_transform.getNumOccurrences()){
    ...
    if(cl_transform_no_dead_code_elim.getNumOccurrences()){
      Debug::warn("Configuration::check_commandline:no:transform:transform-no-dead-code-elim")
        << "WARNING: --no-dead-code-elim ignored in absence of --transform.\n";
    }
